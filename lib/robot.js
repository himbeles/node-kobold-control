...
function Robot(name, serial, secret, token) {
    ...
    this._beehiveBaseUrl = 'https://beehive.ksecosys.com/users/me/robots/'
    this._vorwerkBaseUrl = 'https://nucleo.ksecosys.com:4443/vendors/vorwerk/robots/';
    this.name = name;
    this._serial = serial;
    this._secret = secret;
    this._token = token;
...
function doAction(robot, command, params, callback) {
    ...
    robotRequest(robot, 'vorwerk', 'POST', '/messages', payload, callback);
}
...
function robotRequest(robot, service, type, endpoint, payload, callback) {
    if (robot._serial && robot._secret) {
        ...
        if (service === 'vorwerk') {
            ...
            // cert = __dirname + '/cert/ksecosys.com.crt';

            url = robot._vorwerkBaseUrl + robot._serial + endpoint
        }
        else if (service === 'beehive') {
            headers = {
                Authorization: robot._token
            };
        ...
        api.request(url, payload, type, headers, cert, function (error, body) {
            if (typeof callback === 'function') {
                callback(error, body);
            }
        });
